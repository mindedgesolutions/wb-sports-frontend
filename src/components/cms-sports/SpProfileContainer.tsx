...
const SpProfileContainer = () => {
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  const { currentUserSp } = useAppSelector((store) => store.currentUser);
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild className="ml-0 -mt-3 hidden md:block">
        <Button type="button" variant="ghost" className="focus:outline-none">
          <img
            src={
              user?.user_details.profile_img
                ? `${titles.baseUrl}${user?.user_details.profile_img}`
                : images.profileImg
            }
            alt="user"
      ...
      <DropdownMenuContent align="end" className="mt-1 w-48">
        ...
        <DropdownMenuGroup>
          <Link
            to={`/${titles.sportsUrl}/${user?.user_details?.slug}/settings`}
          >
          ...
          <Link
            to={`/${titles.sportsUrl}/${user?.user_details?.slug}/change-password`}
          >
            <DropdownMenuItem className="cursor-pointer">
...
export default SpProfileContainer;
